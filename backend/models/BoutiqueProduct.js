...
/**
 * BoutiqueProduct Schema
 * Hybrid inventory for Ladies Boutique & Dress Rental.
 * Items can be FOR_SALE, FOR_RENT, or BOTH.
 * Each rental item tracks size, color, condition, and a configurable turnaround time.
 */
...
const boutiqueProductSchema = new mongoose.Schema({
  ...
  // If a product has multiple physical copies, use BoutiqueInventoryUnit
  rentalStatus: {
    type: String,
...
boutiqueProductSchema.index({ tenantId: 1, sku: 1 }, { unique: true });
boutiqueProductSchema.index({ tenantId: 1, category: 1, rentalStatus: 1 });
boutiqueProductSchema.index({ tenantId: 1, mode: 1, rentalStatus: 1 });
boutiqueProductSchema.index({ tenantId: 1, isActive: 1, isFeatured: 1 });

const BoutiqueProduct = mongoose.model('BoutiqueProduct', boutiqueProductSchema);
