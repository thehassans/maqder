  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: '#fff', borderRadius: '16px', maxWidth: '700px', width: '90vw', maxHeight: '85vh',
        ...
        <div style={{ display: 'flex', overflow: 'auto' }}>
          ...
          <div style={{ flex: 1, padding: '24px', minWidth: 0 }}>
            {product.category && <p style={{ fontSize: '12px', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>{product.category}</p>}
            <h2 style={{ fontSize: '20px', fontWeight: 'bold', margin: '0 0 8px' }}>{product.title}</h2>

            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              {product.compareAtPrice && product.compareAtPrice > currentPrice && (
                <span style={{ fontSize: '14px', color: '#dc2626', textDecoration: 'line-through' }}>{product.compareAtPrice} {currency}</span>
              )}
              <span style={{ fontSize: '22px', fontWeight: 'bold', color: '#059669' }}>{currentPrice} {currency}</span>
            </div>

            {product.shortDescription && (
              <p style={{ fontSize: '14px', color: '#4b5563', lineHeight: 1.5, marginBottom: '16px', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                {product.shortDescription}
              </p>
            )}
            ...
            {product.hasVariants && product.variants?.length > 0 && (
              <div style={{ marginBottom: '16px' }}>
                <p style={{ fontWeight: 'bold', fontSize: '13px', marginBottom: '6px' }}>Options:</p>
                  ...
                  {product.variants.filter(v => v.isActive).map(v => {
                    ...
                    return (
                      <button key={v._id} onClick={() => setSelectedVariant(v._id)} disabled={vOutOfStock} style={{
                        padding: '6px 12px', borderRadius: '6px', fontSize: '12px', cursor: vOutOfStock ? 'not-allowed' : 'pointer',
            ...
            {/* Stock status */}
            {outOfStock && <p style={{ fontSize: '13px', color: '#dc2626', fontWeight: 'bold', marginBottom: '12px' }}>Out of stock</p>}

            {/* Quantity + Add to cart */}
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><Minus size={14} /></button>
                <span style={{ padding: '0 12px', fontWeight: 'bold', fontSize: '14px' }}>{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><Plus size={14} /></button>
              </div>
              <button onClick={handleAddToCart} disabled={outOfStock} style={{
                flex: 1, padding: '10px 16px', background: added ? '#059669' : '#4f46e5', color: '#fff', border: 'none', borderRadius: '8px',
            ...
            <Link to={`/store/products/${slug}`} onClick={onClose} style={{
              display: 'inline-flex', alignItems: 'center', gap: '6px', color: '#4f46e5', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px',
            }}>
