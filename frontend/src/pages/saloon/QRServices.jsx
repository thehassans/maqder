...
import { Printer, Download, Scissors, Settings as SettingsIcon, Image as ImageIcon, Save } from 'lucide-react'
...
export default function QRServices() {
  ...
  // Settings State
  const initialSettings = tenant?.settings?.saloon?.qrServices || { defaultLanguage: 'ar', heroImage: '' }
  const [qrSettings, setQrSettings] = useState(initialSettings)
  const [isUploading, setIsUploading] = useState(false)
  const [isSaving, setIsSaving] = useState(false)

  const businessNameEn = tenant?.business?.legalNameEn || tenant?.name || 'Saloon'
  const businessNameAr = tenant?.business?.legalNameAr || tenant?.name || 'صالون'
  
  const servicesUrl = `${window.location.origin}/public/services?tenant=${tenant?._id || ''}`
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      ctx.fillText(isRtl ? 'قائمة الخدمات' : 'Services', canvas.width / 2, 40)
      ctx.font = "20px Arial"
      ...
      downloadLink.download = `QR_Services_${businessNameEn.replace(/\s+/g, '_')}.png`
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 print:hidden">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {isRtl ? 'إدارة كتالوج الخدمات' : 'QR Services Catalog Management'}
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {isRtl ? 'تخصيص وتحميل رمز قائمة الخدمات' : 'Customize and download your QR services catalog'}
          </p>
        </div>
      </div>
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 print:hidden">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-4 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'qr_code'
                ? 'border-indigo-500 text-indigo-600 dark:text-indigo-500 bg-white dark:bg-gray-800'
            ...
            {activeTab === 'qr_code' && (
              ...
              >
                <div className="bg-white rounded-3xl shadow-lg p-10 max-w-sm w-full border border-gray-100 flex flex-col items-center print:shadow-none print:border-none print:p-0">
                  <div className="text-center mb-8">
                    ...
                    <p className="text-gray-500 font-medium">
                      {isRtl ? 'امسح الرمز لعرض الخدمات' : 'Scan to view our services'}
                    </p>
                  </div>
                  ...
                  <div className="w-full border-t border-dashed border-gray-300 pt-6 text-center print:hidden">
                    <p className="text-sm text-gray-500 mb-2">{isRtl ? 'رابط الكتالوج המباشر:' : 'Direct Catalog Link:'}</p>
                    <a 
                      href={servicesUrl} 
                      target="_blank" 
                      rel="noreferrer"
                ...
                <div className="flex gap-3 mt-8 print:hidden">
                  ...
                  <button type="button" onClick={handlePrint} className="btn btn-primary bg-indigo-600 hover:bg-indigo-700">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                      {isRtl ? 'هذه الصورة ستظهر في أعلى القائمة لتضيف طابعاً فخماً للصالون.' : 'This image will appear at the top of the catalog to give it a premium look.'}
                    </p>
                  </div>
