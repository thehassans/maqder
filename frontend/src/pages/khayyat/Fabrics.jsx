    ...
    <div className="space-y-6 animate-fadeIn">
          ...
          <Table>
            <Thead>
              <Tr>
                ...
                <Th>{(language === 'ar' ? 'بلد الصنع' : 'Made In')}</Th>
      ...
      <Modal isOpen={createOpen} onClose={closeCreate} title={(language === 'ar' ? 'إضافة قماش' : 'Add Fabric')} size="lg">
        <div data-tutorial="fabrics-create-modal" className="space-y-4">
          ...
          <Input label={(language === 'ar' ? 'المخزون' : 'Stock')} type="number" min="0" step="1" value={createForm.rollsInStock} onChange={(e) => setCreateForm((p) => ({ ...p, rollsInStock: e.target.value }))} />
      ...
      <Modal isOpen={editOpen} onClose={closeEdit} title={(language === 'ar' ? 'تعديل القماش' : 'Edit Fabric')} size="lg">
        <div className="space-y-4">
          ...
          <Input label={(language === 'ar' ? 'المخزون' : 'Stock')} type="number" min="0" step="1" value={editForm.rollsInStock} onChange={(e) => setEditForm((p) => ({ ...p, rollsInStock: e.target.value }))} />
          <div className="flex gap-3 pt-2">
