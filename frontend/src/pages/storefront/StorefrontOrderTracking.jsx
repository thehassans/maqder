...
export default function StorefrontOrderTracking() {
  ...
  const STATUS_CONFIG = {
    pending: { icon: Clock, color: '#f59e0b', bg: '#fef3c7', label: t('statusPending') },
    confirmed: { icon: CheckCircle, color: '#3b82f6', bg: '#dbeafe', label: t('statusConfirmed') },
    processing: { icon: Package, color: '#8b5cf6', bg: '#ede9fe', label: t('statusProcessing') },
    shipped: { icon: Truck, color: '#06b6d4', bg: '#cffafe', label: t('statusShipped') },
    delivered: { icon: CheckCircle, color: '#059669', bg: '#d1fae5', label: t('statusDelivered') },
    completed: { icon: CheckCircle, color: '#059669', bg: '#d1fae5', label: t('statusCompleted') },
    cancelled: { icon: XCircle, color: '#dc2626', bg: '#fee2e2', label: t('statusCancelled') },
    returned: { icon: XCircle, color: '#dc2626', bg: '#fee2e2', label: t('statusReturned') },
  };

  const handleTrack = async (e) => {
    e.preventDefault();
    if (!form.orderNumber) { setError(t('orderNumberRequired')); return; }
    if (!form.phone && !form.email) { setError(t('phoneOrEmailRequired')); return; }
    setLoading(true);
    setError('');
    setOrder(null);
    try {
      ...
      setError(err.response?.data?.error || t('failedTrackOrder'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMultiLookup = async (e) => {
    ...
    try {
      ...
      setMultiError(err.response?.data?.error || t('noOrdersFound'));
    } finally {
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '40px 20px' }}>
      <StorefrontSeo title={t('trackYourOrder')} />
      <StorefrontBreadcrumbs items={[{ label: t('home'), path: '/store' }, { label: t('trackOrder') }]} />
      <div style={{ textAlign: 'center', marginBottom: '36px' }}>
        ...
        <h1 style={{ fontSize: '30px', fontWeight: 800, marginBottom: '8px', letterSpacing: '-0.5px' }}>{t('trackYourOrder')}</h1>
        <p style={{ color: '#6b7280', fontSize: '15px' }}>{t('trackOrderDesc')}</p>
      </div>
      ...
      <form onSubmit={handleTrack} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '20px', padding: '28px', marginBottom: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
        <div style={{ marginBottom: '16px' }}>
          <label style={{ display: 'block', fontSize: '13px', fontWeight: 700, marginBottom: '6px' }}>{t('orderNumberLabel')}</label>
          <input style={inputStyle} value={form.orderNumber} onChange={e => setForm({ ...form, orderNumber: e.target.value })} placeholder={t('orderNumberPlaceholder')} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
          <div>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: 700, marginBottom: '6px' }}>{t('phone')}</label>
            <input style={inputStyle} value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="05xxxxxxxx" />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: 700, marginBottom: '6px' }}>{t('email')}</label>
        ...
        <p style={{ fontSize: '12px', color: '#9ca3af', marginBottom: '16px' }}>{t('enterPhoneOrEmail')}</p>
        <button type="submit" disabled={loading} style={{
          ...
          {loading ? <Loader2 size={18} className="animate-spin" /> : <><Search size={18} /> {t('trackOrder')}</>}
        </button>
      </form>
      ...
      <div style={{ borderTop: '1px solid #e5e7eb', margin: '24px 0', paddingTop: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
          ...
          <h2 style={{ fontSize: '17px', fontWeight: 700, margin: 0 }}>{t('findAllOrders')}</h2>
        </div>
        <form onSubmit={handleMultiLookup} style={{ display: 'flex', gap: '8px' }}>
          <input style={{ ...inputStyle, flex: 1 }} value={multiPhone} onChange={e => setMultiPhone(e.target.value)} placeholder={t('enterPhoneNumber')} />
          <button type="submit" disabled={multiLoading} style={{ padding: '12px 22px', background: '#6b7280', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 700, fontSize: '14px', cursor: 'pointer', whiteSpace: 'nowrap', opacity: multiLoading ? 0.6 : 1, transition: 'opacity 0.2s' }} onMouseEnter={e => e.currentTarget.style.opacity = '0.9'} onMouseLeave={e => e.currentTarget.style.opacity = '1' }>
            {multiLoading ? <Loader2 size={16} className="animate-spin" /> : t('findOrders')}
          </button>
        </form>
              ...
              return (
                <div key={o.orderNumber} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: '14px', padding: '14px 18px', transition: 'all 0.2s', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }} onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 4px 14px rgba(0,0,0,0.08)'; e.currentTarget.style.transform = 'translateY(-1px)'; }} onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 1px 3px rgba(0,0,0,0.04)'; e.currentTarget.style.transform = 'translateY(0)'; }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#6b7280', margin: '2px 0 0' }}>{new Date(o.createdAt).toLocaleDateString(isRTL ? 'ar' : 'en')} · {o.itemCount} {t('items')} · {o.total} {o.currency}</p>
                  </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '20px', padding: '28px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '24px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '13px', color: '#6b7280', margin: '2px 0 0' }}>{t('orderNumber')} #{order.orderNumber} · {new Date(order.createdAt).toLocaleDateString(isRTL ? 'ar' : 'en')}</p>
            </div>
          </div>

          {/* Order items */}
          <div style={{ marginBottom: '20px' }}>
            <h3 style={{ fontSize: '12px', fontWeight: 700, marginBottom: '10px', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('items')} ({order.itemCount})</h3>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '14px', fontSize: '18px', fontWeight: 800 }}>
              <span>{t('total')}</span><span style={{ color: '#059669' }}>{order.total} {order.currency}</span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
            <div style={{ background: '#f9fafb', borderRadius: '14px', padding: '14px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', margin: '0 0 4px' }}>{t('shipping')}</p>
              <p style={{ fontSize: '14px', fontWeight: 700, margin: 0 }}>{order.shippingStatus}</p>
              {order.trackingNumber && <p style={{ fontSize: '12px', color: '#6b7280', margin: '4px 0 0' }}>{t('trackingLabel')}: {order.trackingNumber}</p>}
            </div>
            <div style={{ background: '#f9fafb', borderRadius: '14px', padding: '14px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', margin: '0 0 4px' }}>{t('paymentMethod')}</p>
          ...
          {order.statusHistory?.length > 0 && (
            <div>
              <h3 style={{ fontSize: '12px', fontWeight: 700, marginBottom: '14px', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('timeline')}</h3>
                  ...
                  return (
                    <div key={i} style={{ position: 'relative', paddingBottom: '18px' }}>
                      {i < order.statusHistory.length - 1 && (
                        <div style={{ position: 'absolute', [isRTL ? 'right' : 'left']: '-17px', top: '24px', bottom: '0', width: '2px', background: '#e5e7eb' }} />
                      )}
                      <div style={{ position: 'absolute', [isRTL ? 'right' : 'left']: '-24px', width: '14px', height: '14px', borderRadius: '50%', background: cfg.color, top: '4px', boxShadow: `0 0 0 4px ${cfg.bg}` }} />
                      <p style={{ fontSize: '14px', fontWeight: 700, margin: 0 }}>{cfg.label}</p>
                      <p style={{ fontSize: '12px', color: '#9ca3af', margin: '2px 0 0' }}>{new Date(h.date).toLocaleString(isRTL ? 'ar' : 'en')}</p>
