...
const INIT = {
  ...
  idType: 'national_id', idNumber: '', idExpiry: '', idPhotoUrl: '',
...
export default function CustomerForm() {
  ...
  const fetchCustomer = useCallback(async () => {
    ...
    try {
      ...
      setForm({
        ...
        idExpiry: data.idExpiry?.split('T')[0] || '', idPhotoUrl: data.idPhotoUrl || '',
  ...
  const section = (title) => (
    <p className="text-xs font-bold uppercase tracking-widest text-amber-600 dark:text-amber-400 mb-3 mt-2 pb-1 border-b border-amber-100 dark:border-amber-900/30">{title}</p>
  )
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-dark-800 rounded-2xl border border-gray-100 dark:border-dark-700 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label={t('Full Name (EN)', 'الاسم الكامل (إنج)')} required><Input value={form.fullName} onChange={set('fullName')} /></Field>
          <Field label={t('Full Name (AR)', 'الاسم الكامل (ع)')}><Input value={form.fullNameAr} onChange={set('fullNameAr')} dir="rtl" /></Field>
